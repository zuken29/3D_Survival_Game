...
public class ConstructionManager : Singleton<ConstructionManager>
{
    ...
    private void Update()
    {
        ...
        if (Input.GetMouseButtonDown(0) && inConstructionMode)
        {
            if (isValidPlacement && selectedGhost == false &&
                itemToBeConstructed.name ==
                "FoundationModel") // We don't want the freestyle to be triggered when we select a ghost.
            {
                PlaceItemFreeStyle();
    ...
    private void PlaceItemInGhostPosition(GameObject copyOfGhost)
    {
        ...
        itemToBeConstructed.transform.position =
            new Vector3(ghostPosition.x, ghostPosition.y, ghostPosition.z + randomOffset);
